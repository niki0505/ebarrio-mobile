...
const CourtReservations = () => {
  ...
  useEffect(() => {
    const calculateAmount = () => {
      ...
      const timeDifference = reservationForm.datetime - currentDate; // difference in milliseconds
      const hoursDifference = Math.max(timeDifference / (1000 * 3600), 0); // Convert milliseconds to hours
      const calculatedAmount = hoursDifference * hourlyRate;
      const amount = Math.round(calculateAmount);
      setReservationForm((prev) => ({
        ...prev,
        amount: amount, // Round to nearest integer
      }));
    };
